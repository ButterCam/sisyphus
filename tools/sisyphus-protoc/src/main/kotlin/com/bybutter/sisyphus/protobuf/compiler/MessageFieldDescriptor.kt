...
open class MessageFieldDescriptor(
    ...
    fun path(): List<Int> {
        return parent.path() + DescriptorProtos.DescriptorProto.FIELD_FIELD_NUMBER +
            parent.descriptor.fieldList.indexOf(
                descriptor,
            )
    }
...
fun MessageFieldDescriptor.hidden(): Boolean {
    val conflictedField =
        descriptor.name.substringAfter("is_", "").takeIf { it.isNotBlank() }
...
fun DescriptorNode<DescriptorProtos.FieldDescriptorProto>.defaultValue(): CodeBlock =
    buildCodeBlock {
...
fun DescriptorNode<DescriptorProtos.FieldDescriptorProto>.elementType(): TypeName {
    return when (descriptor.type) {
        ...
        DescriptorProtos.FieldDescriptorProto.Type.TYPE_GROUP -> throw UnsupportedOperationException(
            "Group is not supported by butter proto.",
        )
        ...
        DescriptorProtos.FieldDescriptorProto.Type.TYPE_ENUM ->
            fileSet().ensureEnum(descriptor.typeName)
                .className()
    }
}
