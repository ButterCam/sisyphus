...
object DaemonSetSupport : KubernetesResourceSupport<V1DaemonSet> {
    ...
    override fun patchResourceCall(resource: KubernetesResource<V1DaemonSet>, patch: V1Patch): Call {
    ...
    override fun updateMetadata(resource: KubernetesResource<V1DaemonSet>, k8sResource: V1DaemonSet) {
    ...
    override fun checkReady(resource: KubernetesResource<V1DaemonSet>, task: Task, k8sResource: V1DaemonSet): Boolean {
        task.logger.lifecycle("Waiting for ${GUtil.toCamelCase(kind)} ${resource.namespace}/${resource.name} to be ready(${k8sResource.status?.numberUnavailable}/${k8sResource.status?.numberAvailable}).")
