...
class RegisterTest {
    @Test
    fun `test client proxy by service autowiring`(
        @Autowired client: RpcService.Client,
    ) {
        runBlocking {
            client.sayHello(
                Request {
...
class ServiceImpl : RpcService() {
    ...
    override fun sayHelloStream(input: Request): Flow<Response> =
        flow {
            emit(
                Response {
                    value = "test1"
    ...
    override fun sayHelloStream3(input: Flow<Request>): Flow<Response> =
        flow {
            input.collect {
                emit(
                    Response {
