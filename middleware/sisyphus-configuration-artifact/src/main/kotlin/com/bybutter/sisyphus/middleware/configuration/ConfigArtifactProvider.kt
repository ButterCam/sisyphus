...
class ConfigArtifactProvider : EnvironmentPostProcessor, ApplicationListener<ApplicationPreparedEvent> {
    override fun postProcessEnvironment(
        ...
        val properties =
            binder.bind("sisyphus", SisyphusProperty::class.java)
                .orElse(null) ?: return
        ...
        for (repositoryKey in properties.dependency.repositories) {
            val repository =
                when (repositoryKey) {
                    "local" ->
                        properties.repositories[repositoryKey] ?: run {
        ...
        val artifacts =
            properties.config.artifacts.flatMap {
                try {
        ...
        val propertySources =
            ServiceLoader.load(ConfigArtifactPropertyExporter::class.java)
