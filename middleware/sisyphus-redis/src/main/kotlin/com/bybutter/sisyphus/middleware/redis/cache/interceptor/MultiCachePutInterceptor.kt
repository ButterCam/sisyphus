...
class MultiCachePutInterceptor(
    ...
    private fun normalCachePut(
        invocation: MethodInvocation,
        method: Method,
        multiCachePut: MultiCachePut,
        multiRedisCache: Cache?
    ): Any? {
        val key = generateNormalKey(invocation, multiCachePut.key, method, evaluationContextInterceptorList)?.toString()
            ?: return invocation.proceed()
        return cache(key, invocation, multiRedisCache)
    }

    private fun globalCachePut(
        invocation: MethodInvocation,
        method: Method,
        multiCachePut: MultiCachePut,
        multiRedisCache: Cache?
