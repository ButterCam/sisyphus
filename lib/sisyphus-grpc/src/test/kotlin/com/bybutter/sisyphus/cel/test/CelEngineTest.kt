package com.bybutter.sisyphus.cel.test


import com.bybutter.sisyphus.api.cel.test.CellTest
import com.bybutter.sisyphus.cel.*
...
class CelEngineTest {
    ...
    fun `test times cel eval`() {
        val engine = CelEngine(mapOf())
        engine.assertEvalResult("""timestamp('2020-01-01T08:00:00.000Z') + duration('100.0s')""", Timestamp(2020, 1, 1, 16, 1, 40))
        engine.assertEvalResult("""duration('100.0s') + timestamp('2020-01-01T08:00:00.000Z')""", Timestamp(2020, 1, 1, 16, 1, 40))
        engine.assertEvalResult("""timestamp('2020-01-01T08:00:00.000Z') - duration('100.0s')""", Timestamp(2020, 1, 1, 15, 58, 20))
        engine.assertEvalResult("""duration('100.0s') + duration('100.0s')""", Duration(200.0, TimeUnit.SECONDS))
        engine.assertEvalResult("""timestamp('2020-01-01T08:00:00.000Z') - timestamp('2020-01-01T07:00:00.000Z')""", Duration(3600.0, TimeUnit.SECONDS))
    ...
    fun `test compare cel eval`() {
        ...
        engine.assertEvalResult(""" 'abc' == 'abcd'""", false) //
        engine.assertEvalResult("""timestamp('2020-01-01T08:00:00.000Z') > timestamp('2020-01-02T08:00:00.000Z') """, false)
//        engine.assertEvalResult("""timestamp('2020-01-01T08:00:00.000Z') == timestamp('2020-01-01T08:00:00.000Z') """,Timestamp())
        engine.assertEvalResult("""timestamp('2020-01-01T08:00:00.000Z') < timestamp('2020-01-02T08:00:00.000Z') """, true)
//        engine.assertEvalResult("""duration('100.0s') == duration('100.0s')""",Duration())
        ...
        val result = engine.eval(""" .sisyphus.api.cel.test.ConditionalTest {
            | condition: true
            |} """.trimMargin())
        result
    }
    ...
    fun `test get time from the date`() {
        val engine = CelEngine(mapOf(
            "value" to Timestamp(2020, 1, 1, 16, 1, 1, 999),
            "value2" to Duration(3600.0, TimeUnit.SECONDS),
            "zone" to "UTC"
        ))
        engine.assertEvalResult("""getDayOfMonth(value)""", 0L)
//        engine.assertEvalResult("""getDayOfMonth(zone)""",0L)
        engine.assertEvalResult("""getDayOfWeek(value)""", 3L)
        engine.assertEvalResult("""getDayOfYear(value)""", 0L)
//        engine.assertEvalResult("""getDayOfYear(zone)""",0L)
        engine.assertEvalResult("""getFullYear(value)""", 2020L)
//        engine.assertEvalResult("""getFullYear(zone)""",0L)
        engine.assertEvalResult("""getHours(value)""", 8L)
//        engine.assertEvalResult("""getHours(zone)""",16L)
        engine.assertEvalResult("""getHours(value2)""", 1L)
        engine.assertEvalResult("""getMilliseconds(value)""", 0L)
//        engine.assertEvalResult("""getMilliseconds(zone)""",0L)
        engine.assertEvalResult("""getMilliseconds(value2)""", 0L)
        engine.assertEvalResult("""getMinutes(value)""", 1L)
//        engine.assertEvalResult("""getMinutes(zone)""",1L)
        engine.assertEvalResult("""getMinutes(value2)""", 60L)
        engine.assertEvalResult("""getMonth(value)""", 0L)
//        engine.assertEvalResult("""getMonth(zone)""",1L)
        engine.assertEvalResult("""getSeconds(value)""", 1L)
//        engine.assertEvalResult("""getSeconds(zone)""",1L)
        engine.assertEvalResult("""getSeconds(value2)""", 3600L)
    }
    ...
    fun `test List CelRuntime with macro `() {
        ...
        val result3 = engine.eval("""has(4)""")
        val result4 = engine.eval("""has(1)""")
        val result5 = engine.eval("""value.exists(x,x >= 1)""")
        val result6 = engine.eval("""value.exists(x,x > 1 && x < -1)""")
        val result7 = engine.eval("""value.exists_one(x,x + 1 == 2)""")
        val result8 = engine.eval("""value.exists_one(x,x > 3)""")
        val result9 = engine.eval("""value.map(x,x == 1)""")
        val result10 = engine.eval("""value.map(x,x * x)""")
        val result11 = engine.eval("""value.filter(x,x % 2 > 0)""")
        val result12 = engine.eval("""value.filter(x,x > 3""")
        ...
        Assertions.assertEquals(result9, arrayListOf(true, false))
        Assertions.assertEquals(result10, arrayListOf(1L, 9L))
        Assertions.assertEquals(result11, arrayListOf(1L, 3L))
        Assertions.assertEquals(result12, arrayListOf(1))
    }
    ...
    fun `test map CelRuntime with macro`() {
        ...
        val result = engine.eval("""value.all(x,x == 1)""")
        val result2 = engine.eval("""value.all(x,x != 3)""")
        val result3 = engine.eval("""has(2)""")
        val result4 = engine.eval("""has(4)""")
        val result5 = engine.eval("""value.exists(x,x == 5)""")
        val result6 = engine.eval("""value.exists(x,x == 2)""")
        val result7 = engine.eval("""value.exists_one(x,x == 2)""")
        val result8 = engine.eval("""value.exists_one(x,x == 3)""")
    ...
    class CustomCelMacro : CelMacro() {
        override fun has(context: CelContext, expr: CelParser.ExprContext): Boolean {
            return true
        }
    }
