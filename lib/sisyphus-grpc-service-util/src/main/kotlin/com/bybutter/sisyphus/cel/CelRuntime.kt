...
open class CelRuntime(val macro: CelMacro = CelMacro(), val std: CelStandardLibrary = CelStandardLibrary()) {
    ...
    fun getGlobalField(key: String, global: Map<String, Any?>): Any? {
        return when (key) {
            ...
            "null_type" -> "null_type"
                ...
                if (key.startsWith(".")) {
                    ProtoTypes.findSupport(key)?.let { return it.name }
                }
    ...
    fun createMessage(type: String, initializer: Map<String, Any?>): Any {
        val messageSupport = ProtoTypes.findSupport(type) as? MessageSupport<*, *> ?: throw IllegalStateException("Message '$type' not defined.")
