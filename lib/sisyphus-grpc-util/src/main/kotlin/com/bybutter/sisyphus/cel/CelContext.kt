    ...
    fun visit(member: CelParser.MemberContext): Any? {
        return when (member) {
            ...
            is CelParser.SelectOrCallContext -> {
                if (member.open != null) {
                    engine.runtime.invokeMarco(this, visit(member.member()), member.IDENTIFIER().text, member.args?.e
                        ?: listOf()) { return it }
                    engine.runtime.invoke(visit(member.member()), member.IDENTIFIER().text, member.args?.e?.map { visit(it) }
                        ?: listOf())
                } else {
    ...
    fun visit(primary: CelParser.PrimaryContext): Any? {
        return when (primary) {
            is CelParser.IdentOrGlobalCallContext -> {
                if (primary.op != null) {
                    engine.runtime.invokeMarco(this, null, primary.IDENTIFIER().text, primary.args?.e
                        ?: listOf()) { return it }
